...
from ..utils import inference_greedy, inference_beam, inference_top_p, inference_top_k,save_xyz_file
...
@register_model("geoldm")
class GEOLDM(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def __init__vae(self, args):
        ...
        nodes_dist = DistributionNodes(HISTOGRAM[self.args.task_name])
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.args.include_charges:
            ...
            charges = torch.zeros(0).to(src_coord.device, src_coord.dtype)
        ...
        node_mask = mask.to(src_coord.device, src_coord.dtype).unsqueeze(2)

        # edge mask
        edge_mask = mask.unsqueeze(1) * mask.unsqueeze(2)
        diag_mask = ~torch.eye(edge_mask.size(1), dtype=torch.bool).unsqueeze(0)
        edge_mask *= diag_mask.to(edge_mask)
        edge_mask = edge_mask.view(batch_size*n_nodes*n_nodes, 1).to(src_coord.device, src_coord.dtype)
    ...
    def sample_chain(self, n_tries, prop_dist=None):
        ...
        save_xyz_file(f'{self.args.save_dir}/outputs/{self.args.task_name}/sample_chain/chain/',
                      one_hot, charges, x, self.dictionary, 0, name='chain')
    ...
    def sample(
        ...
        device = self.embed_tokens.weight.device
        max_n_nodes =  max(HISTOGRAM[self.args.task_name].keys()) # this is the maximum node_size in QM9 #29
        ...
        if hasattr(self.args, 'save_dir'):
            save_xyz_file(f'{self.args.save_dir}/outputs/{self.args.task_name}/sample_molecule/', one_hot, charges, x, self.dictionary, 0, name='molecule')
...
@register_model_architecture("geoldm", "geoldm")
...
@register_model_architecture("geoldm", "geoldm-qm9")
def base_architecture(args):
    # encoder
...
@register_model_architecture("geoldm", "geoldm-oled")
def base_architecture(args):
    # encoder
    args.encoder_layers = getattr(args, "encoder_layers", 4) #8
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 2048)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 64)
    args.dropout = getattr(args, "dropout", 0.1)
    ...
    # decoder
    args.decoder = getattr(args, "decoder", "TFM")
    args.decoder_layers = getattr(args, "decoder_layers", 4) # 9
    args.embed_dim = getattr(args, "embed_dim", 512)
    args.ffn_embed_dim = getattr(args, "ffn_embed_dim", 2048)
    args.attention_heads = getattr(args, "attention_heads", 64)
    args.rel_pos = getattr(args, "rel_pos", True)
...
@register_model_architecture("geoldm", "geoldm-opv")
def base_architecture(args):
    # encoder
