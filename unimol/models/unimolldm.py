...
from ..utils import inference_greedy, inference_beam, inference_top_p, inference_top_k,check_stabilityori, save_xyz_fileori
...
class EnHierarchicalVAE(torch.nn.Module):
    ...
    def __init__(
            ...
            encoder,
            decoder,
            in_node_nf: int, n_dims: int, latent_node_nf: int, hidden_dim: int,
            kl_weight: float,
        ...
        self.final_mlp = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Linear(hidden_dim * 2, latent_node_nf * 2 + 1))
    ...
    def _forward(self, encoder=None, node_mask=None, x_coord=None):
        bs, n_nodes, _ = node_mask.size()

        h_final = self.final_mlp(encoder)
        h_final = h_final * node_mask  if node_mask is not None else h_final

        # x_final = self.coord_mlp(encoder) # TODO: Clofnet
        x_final = x_coord
        ...
        h_mean = h_final[:, :, 1:1 + self.latent_node_nf]
        h_std = torch.exp(0.5 * h_final[:, :, 1 + self.latent_node_nf:])
    ...
    def encode(self, x, h, node_mask=None, edge_mask=None, context=None, encoder=None, x_coord=None):
        """Computes q(z|x)."""
        # mean mask 
        encoder = remove_mean_with_mask(encoder, node_mask)
        x_coord = remove_mean_with_mask(x_coord, node_mask)
        assert_mean_zero_with_mask(encoder, node_mask)
        assert_mean_zero_with_mask(x_coord, node_mask)
        # Encoder output.

        z_x_mu, z_x_sigma, z_h_mu, z_h_sigma = self._forward(encoder, node_mask, x_coord)
...
class EnLatentDiffusion(EnVariationalDiffusion):
    ...
    def __init__(self, **kwargs):
        ...
        trainable_encoder = kwargs.pop('trainable_encoder', False)
        distance_loss = kwargs.pop('distance_loss', False)
    ...
    def compute_distance(self, coord, node_mask):
        diff = coord.unsqueeze(2) - coord.unsqueeze(1)
        dist = torch.sum(diff ** 2, dim=-1)
        dist = (dist + 1e-10).sqrt()
        masked_dist = (dist * (node_mask.squeeze(-1).unsqueeze(1) * node_mask.squeeze(-1).unsqueeze(2)))
        return masked_dist

    def forward(self, x, h, node_mask=None, edge_mask=None, context=None, encoder=None, x_coord=None):
        """
        Computes the loss (type l2 or NLL) if training. And if eval then always computes NLL.
        """

        # Encode data to latent space.
        z_x_mu, z_x_sigma, z_h_mu, z_h_sigma = self.vae.encode(x, h, node_mask, edge_mask, context, encoder, x_coord) # torch.Size([64, 23, 3]) torch.Size([64, 23, 5])
        ...
        if self.trainable_ae:
            xh = torch.cat([x, h['categorical'], h['integer']], dim=2)
            # Decoder output (reconstruction).
            x_recon, h_recon = self.vae.decoder._forward(z_xh, node_mask, edge_mask, context) # torch.Size([8, 24, 4])

            # TODO: Add distance
            if self.distance_loss:
                distance_recon = self.compute_distance(x_recon.clone(), node_mask) #mask
                distance = self.compute_distance(xh[:, :, :self.n_dims].clone(), node_mask) #mask
                loss_distance = sum_except_batch((distance_recon - distance) ** 2)
            else:
                loss_distance = 0
            xh_rec = torch.cat([x_recon, h_recon], dim=2)
        ...
        neg_log_constants = -self.log_constants_p_h_given_z0(
            torch.cat([h['categorical'], h['integer']], dim=2), node_mask) # add 0.1 * 0.1
        # Reset constants during training with l2 loss.
        if self.training and self.loss_type == 'l2':
            neg_log_constants = torch.zeros_like(neg_log_constants)

        neg_log_pxh = loss_ld + loss_recon + neg_log_constants + loss_distance

        return neg_log_pxh, loss_ld, loss_recon, neg_log_constants, loss_distance
...
@register_model("unimolldm")
class UnimolLDM(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def __init__(self, args, dictionary):
        ...
        first_stage_model, self.nodes_dist, self.prop_dist = self.__init__vae(args)
        in_node_nf = self.latent_dim

        # self.se3_coord = DECODER_REGISTER[args.decoder](self.args.encoder_attention_heads, self.args.activation_fn, args.net_dynamics_encoder_layers)
        self.se3_coords = nn.ModuleList([
                    DECODER_REGISTER[args.decoder](self.args.encoder_attention_heads, self.args.activation_fn)
                    for _ in range(self.args.net_dynamics_encoder_layers)
                ])       
        self.method_dict = {
        ...
        context_node_nf = 0
        args.context_node_nf = context_node_nf # TODO: Add condition
        net_dynamics = EGNN_dynamics_QM9(
            in_node_nf=dynamics_in_node_nf, 
            context_node_nf=args.context_node_nf, #2 0 
            n_dims=3, 
            hidden_nf=args.embed_dim, # 256
            act_fn=torch.nn.SiLU(), 
            n_layers=args.net_dynamics_encoder_layers, # 9
            attention=True, 
            tanh=True, 
            mode='egnn_dynamics', 
            norm_constant=1,
        ...
        self.vdm = EnLatentDiffusion(
            ...
            trainable_encoder=args.trainable_encoder if hasattr(args, "trainable_encoder") else False,
            distance_loss=args.distance_loss if hasattr(args, "distance_loss") else False,            
            ...
            include_charges=self.args.include_charges, # True
            )

    def __init__vae(self, args):

        in_node_nf = len(self.dictionary) + int(self.args.include_charges)
        nodes_dist = DistributionNodes(HISTOGRAM[self.args.task_name]['n_nodes'])
        context_node_nf = 0 # TODO: Add condition
        self.context_node_nf = context_node_nf
        prop_dist = None
        # if len(args.conditioning) > 0: # TODO: Add condition
        ...
        # )
        self.encoder = ENCODER_REGISTER[args.encoder](self.args, self.dictionary)

        decoder = EGNN_decoder_QM9(
            in_node_nf=self.latent_dim, 
            context_node_nf=context_node_nf, 
            out_node_nf=in_node_nf, # 1 0 5 
            n_dims=3, 
            hidden_nf=args.embed_dim, # 256
            act_fn=torch.nn.SiLU(), 
    ...
    def compute_pred_pos_clof(self, pos_perturbed, encoder_outputs, **kwargs):
        ...
        for se3_coord_layer in self.se3_coords:
            pos_perturbed = se3_coord_layer(pos_perturbed, encoder_outputs[2], padding_mask)
    ...
    def run_model(self, src_coord, src_tokens, kwargs, padding_mask):
        encoder_outputs = self.encoder(src_tokens=src_tokens, src_coord=src_coord, **kwargs)
        src_coord = self.method_dict[self.args.decoder](src_coord, encoder_outputs, **{'padding_mask': padding_mask})
        return src_coord, encoder_outputs
    ...
    def forward(
        ...
        **kwargs
        ...
        node_mask = atom_mask.to(src_coord.device, src_coord.dtype).unsqueeze(2)
        edge_mask = edge_mask.to(src_coord.device, src_coord.dtype)
        charges = (charges if self.args.include_charges else torch.zeros(0)).to(src_coord.device, src_coord.dtype)
        one_hot = F.one_hot(src_tokens, self.num_classes)*node_mask
        one_hot = one_hot.to(src_coord.device, src_coord.dtype)

        padding_mask = ~atom_mask


        if hasattr(self.args, "recycling"):
            if self.training:
                np.random.seed(42)
                recycling = np.random.randint(2, 5)
                for _ in range(recycling):
                    with torch.no_grad():
                        src_coord, encoder_outputs = self.run_model(src_coord, src_tokens, kwargs, padding_mask)
                
                clof_output, encoder_outputs = self.run_model(src_coord, src_tokens, kwargs, padding_mask)
            else:
                with torch.no_grad():
                    for _ in range(4):
                        src_coord, encoder_outputs = self.run_model(src_coord, src_tokens, kwargs, padding_mask)
                clof_output = src_coord
        else:
            encoder_outputs = self.encoder(src_tokens=src_tokens, src_coord=src_coord, **kwargs)
            clof_output = self.method_dict[self.args.decoder](src_coord, encoder_outputs, **{'padding_mask': padding_mask})

        assert_correctly_masked(clof_output, node_mask)
        assert_correctly_masked(src_coord, node_mask)
        x = remove_mean_with_mask(src_coord, node_mask)
        assert_mean_zero_with_mask(x, node_mask)
        ...
        nll, loss_ld, loss_recon, neg_log_constants, distance_loss = self.vdm(x, h, node_mask, edge_mask.view(batch_size, n_nodes * n_nodes), context, encoder=encoder_outputs[0]*node_mask, x_coord=clof_output*node_mask)
        ...
        return {
            ...
            "loss_distance": distance_loss.mean() if hasattr(self.args, 'distance_loss') else 0.0,
        }



    @torch.no_grad()
    def sample_chain(self, n_tries, prop_dist=None):
        ...
        if self.args.task_name == 'qm9':
            ...
            n_nodes = 19
        elif self.args.task_name == 'oled_geoldm' or self.args.task_name == 'oled':
            n_nodes = 41
        else:
            raise ValueError()
        ...
        for i in range(n_tries):
            ...
            mol_stable = check_stabilityori(x_squeeze, atom_type, dataset_info, task_name=self.args.task_name)[0]
        ...
        save_xyz_fileori(f'{self.args.save_dir}/outputs/{self.args.task_name}/sample_chain/chain/',
                      one_hot, charges, x, dataset_info, 0, name='chain')
    ...
    def sample(
        ...
        if hasattr(self.args, 'save_dir'):
            save_xyz_fileori(f'{self.args.save_dir}/outputs/{self.args.task_name}/sample_molecule/', one_hot, charges, x, dataset_info, 0, name='molecule')

        return one_hot, charges, x, node_mask

    @torch.no_grad()
    def sample_sweep_conditional(self, args, device, generative_model, dataset_info, prop_dist, n_nodes=19, n_frames=100):
        ...
        one_hot, charges, x, node_mask = self.sample(args, device, generative_model, dataset_info, prop_dist, nodesxsample=nodesxsample, context=context, fix_noise=True)
        return one_hot, charges, x, node_mask
...
@register_model_architecture("unimolldm", "unimolldm")
...
@register_model_architecture("unimolldm", "unimolldm-qm9")
def base_architecture(args):
    # encoder
    args.encoder_layers = getattr(args, "encoder_layers", 9) #6 8 2
    args.net_dynamics_encoder_layers = getattr(args, "net_dynamics_encoder_layers", 9) # 9
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512) # 512
    args.embed_dim = getattr(args, "embed_dim", 256) # 512    
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 2048)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 64)
    args.dropout = getattr(args, "dropout", 0.1)
    args.emb_dropout = getattr(args, "emb_dropout", 0.1)
    ...
    # decoder
    args.decoder = getattr(args, "decoder", "TFM")
    args.decoder_layers = getattr(args, "decoder_layers", 9) # 9
    args.embed_dim = getattr(args, "embed_dim", 512)
    args.ffn_embed_dim = getattr(args, "ffn_embed_dim", 2048)
    args.attention_heads = getattr(args, "attention_heads", 64)
    args.rel_pos = getattr(args, "rel_pos", True)
...
@register_model_architecture("unimolldm", "unimolldm-oled")
def base_architecture(args):
    # encoder
    args.encoder_layers = getattr(args, "encoder_layers", 3) # 8
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.embed_dim = getattr(args, "embed_dim", 256) # 512 
    args.net_dynamics_encoder_layers = getattr(args, "net_dynamics_encoder_layers", 6) # 9
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 2048)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 64)
    args.dropout = getattr(args, "dropout", 0.1)
    args.emb_dropout = getattr(args, "emb_dropout", 0.1)
...
@register_model_architecture("unimolldm", "unimolldm-opv")
def base_architecture(args):
    # encoder
