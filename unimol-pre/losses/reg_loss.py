    ...
    def forward(self, model, sample, reduce=True):
        ...
        if not self.training:
            ...
            logging_output = {
                ...
                "target": sample["target"]["finetune_target"][:, :self.args.num_classes]
                .view(-1, self.args.num_classes)
                .data,
                "name": [name.replace('_xtb', '') for name in sample["name"]],
...
class FinetuneSmoothMAELoss(FinetuneMSELoss):
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        predicts = net_output.view(-1, self.args.num_classes).float()
        targets = (
            sample["target"]["finetune_target"][:, :self.args.num_classes].view(-1, self.args.num_classes).float()
        )
