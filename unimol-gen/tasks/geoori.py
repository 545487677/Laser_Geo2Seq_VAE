...
@register_task("geoldmori")
class UniMolFinetunegeoldmoriTask(UnicoreTask):
    ...
    def load_dataset(self, split, **kwargs):
        ...
        dataset = RemoveHydrogenDataset(dataset, 'atoms', 'coordinates', self.args.remove_hydrogen, None, self.dictionary)
        one_hot_dataset = KeyDataset(dataset, "atoms")
        one_hot_dataset = RawListDataset(one_hot_dataset)

        # condition dataset
        condition_datasets = {}
        for prop in self.args.conditioning:
            condition_datasets[prop] = KeyDataset(dataset, prop)
        ...
        # dataset = NormalizeDataset(dataset, "coordinates", normalize_coord=True) ##???????TODO
        ...
        #     src_dataset, None, app_token=self.dictionary.eos() # self.dictionary.bos()
        # )
        # edge_type = EdgeTypeDataset(src_dataset, len(self.dictionary))        
        coord_dataset = FromNumpyDataset(coord_dataset)
        # coord_dataset = PrependAndAppend(coord_dataset, None, 0.0) #0.0 0.0  rm eos
        # one_hot_dataset = PrependAndAppend(one_hot_dataset, None, 0.0)
        ...
        nest_dataset = NestedDictionaryDataset(
            {
                "net_input": {
                     ...
                     **{k: condition_datasets[k] for k in self.args.conditioning}
                    # "charge_dict": charge_dict_dataset,
                },
            },
        )
