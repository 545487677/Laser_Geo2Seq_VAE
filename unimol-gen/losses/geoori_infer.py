...
@register_loss("finetune_geoldmori_infer")
class FinetunegeoldmoriInferLoss(CrossEntropyLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if len(self.task.args.conditioning) > 0:
            smi_list = []
            for i in range(self.task.args.n_sweeps):
                logger.info("Sampling sweep %d/%d" % (i+1, self.task.args.n_sweeps))    
                smi_list_ = self.process_condition_samples(model, i, HISTOGRAM[self.task.args.task_name], xyz2smi)
                smi_list.extend(smi_list_)
            logging_output = {
    ...
    def inference(self, model, bsz, dictionary, n_samples):
        ...
        for i in range(int(n_samples/batch_size)):
            ...
            save_xyz_fileori(f'{self.task.args.results_path}/outputs/{self.task.args.task_name}/sample_molecule/', one_hot, charges, x, HISTOGRAM[self.task.args.task_name], id_from, name='molecule')

        molecules = {key: torch.cat(molecules[key], dim=0) for key in molecules}

        smi_list, atoms_list, coord_list, valid_dict, mol_list = self.compute_molecule_stability(molecules, HISTOGRAM[self.task.args.task_name], check_stabilityori, self.task.args.task_name)
    ...
    def process_condition_samples(self, model, i, HISTOGRAM, xyz2smi):
        one_hot, charges, x, node_mask = model.sample_sweep_conditional(model.prop_dist, i=i)
        ...
        for idx in range(n_samples):
            atom_type = one_hot[idx].argmax(1).cpu().detach()
            pos = x[idx].cpu().detach()
            atom_type = atom_type[0:int(atomsxmol[idx])]
            pos = pos[0:int(atomsxmol[idx])]
            processed_list.append((pos, atom_type))

        for mol in processed_list:
            pos, atom_type = mol
            atoms = [HISTOGRAM['atom_decoder'][item.item()] for item in atom_type.numpy()]
            smi = xyz2smi(atoms, pos.numpy())
            if '.' not in smi:
                smi_list.append(smi)

        save_xyz_fileori(f'{self.task.args.results_path}/outputs/{self.task.args.task_name}/sample_condition/', one_hot, charges, x, HISTOGRAM, i, name='condition')
