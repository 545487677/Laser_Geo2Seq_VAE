...
@register_model("unimol-oled")
class UniMolOLEDModel(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def forward(
        ...
        **kwargs
        ...
        def get_dist_features(dist, et, pair_charge):
            n_node = dist.size(-1)
            gbf_feature = self.gbf(x=dist, edge_type=et, pair_charge=pair_charge)
        ...
        graph_attn_bias = get_dist_features(src_distance, src_edge_type, src_pair_charge)
...
@register_model_architecture("unimol", "unimol-oled")
