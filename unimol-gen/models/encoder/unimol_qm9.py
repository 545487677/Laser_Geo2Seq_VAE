...
@register_model("unimol-qm9")
class UniMolQM9Model(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def forward(
        ...
        **kwargs
        ...
        outputs = self.encoder(x, padding_mask=padding_mask, attn_mask=graph_attn_bias)
        return outputs
...
class SE3CoordHead(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        logger.info('SE3')
        self.pair2coord_proj = NonLinearHead(heads, 1, activation_fn,  hidden=hidden)

    def forward(self, src_coord, delta_encoder_pair_rep, padding_mask=None):
        # Note: padding_mask contain cls
        if padding_mask is not None:
            atom_num = (torch.sum(1-padding_mask.type_as(src_coord), dim=1)).view(-1, 1, 1, 1)
            nonpadding_mask = (~padding_mask).to(src_coord)[..., None, None]
        else:
        ...
        coord_update = delta_pos * torch.tensor(nonpadding_mask).to(src_coord) * attn_probs
        coord_update = torch.sum(coord_update / atom_num, dim=2)
        encoder_coord = src_coord + coord_update
        return encoder_coord


class SE3CoordClofHead(nn.Module):
    def __init__(
        self,
        heads,
        activation_fn,
        net_dynamics_encoder_layers=8,
        hidden=512,
        **kwargs
    ):
        super().__init__()

        self.pair2coord_proj = NonLinearHead(heads, 3, activation_fn,  hidden=hidden)

    @staticmethod
    def local_equivariant_frame(xi, xj):
        # Compute a_ij
        a_ij = xi - xj
        a_ij_norm = torch.norm(a_ij, dim=-1, keepdim=True)
        a_ij = a_ij / (a_ij_norm + 1e-8)  # Adding a small epsilon for numerical stability

        # Compute b_ij
        b_ij = torch.cross(xi, xj) + 1e-8
        b_ij_norm = torch.norm(b_ij, dim=-1, keepdim=True)
        b_ij = b_ij / (b_ij_norm + 1e-8)

        # Compute c_ij
        c_ij = torch.cross(a_ij, b_ij)  + 1e-8

        return a_ij, b_ij, c_ij
    
    def forward(self, src_coord, delta_encoder_pair_rep, padding_mask=None):
        # Note: padding_mask contain cls
        if padding_mask is not None:
            atom_num = (torch.sum(1-padding_mask.type_as(src_coord), dim=1)).view(-1, 1, 1, 1, 1)
            nonpadding_mask = (~padding_mask).to(src_coord)[..., None, None, None]
        else:
            atom_num = src_coord.shape[1]
            nonpadding_mask = 1

        a_ij, b_ij, c_ij = self.local_equivariant_frame(src_coord.unsqueeze(1), src_coord.unsqueeze(2)) 
        delta_pos = torch.stack([a_ij, b_ij, c_ij], dim=-2) # b, n, n, 3(abc), 3
        
        attn_probs = self.pair2coord_proj(delta_encoder_pair_rep).unsqueeze(-1) # b, n, n, 3, 1
        # coord_update = delta_pos / atom_num * attn_probs
        # coord_update = torch.sum(coord_update, dim=2)
        coord_update = delta_pos * nonpadding_mask.clone().detach().to(src_coord) * attn_probs # b, n, n, 3, 3
        coord_update = torch.sum(coord_update / (atom_num + 1e-8), dim=(2, 3))
        encoder_coord = src_coord + coord_update
        return encoder_coord
        ...
        #     attn_probs = self.pair2coord_proj(delta_encoder_pair_rep).unsqueeze(-1) # b, n, n, 3, 1
        #     coord_update = delta_pos * nonpadding_mask.clone().detach().to(src_coord) * attn_probs # b, n, n, 3, 3
        #     coord_update = torch.sum(coord_update /  (atom_num + 1e-4), dim=(2, 3))
        #     src_coord = src_coord + coord_update
...
@register_model_architecture("unimol", "unimol-qm9")
def base_architecture(args):
    ...
    args.kernel_size = getattr(args, "kernel_size", 128)

@register_model_architecture("unimol", "unimol_qm9_small")
def unimol_opv_architecture(args):
    base_architecture(args)
