...
from .encoder import UniMolV1Model, UniMolOLEDModel, UniMolOPVModel, UniMolLASERModel
...
class Vae(BaseUnicoreModel):
    ...
    def __init__(self, args, dictionary, smi_dictionary):
        ...
        self.mol_vae_model = Molecule_VAE(
    ...
    def forward(
        ...
        **kwargs
        ...
        output, mean, logvar = self.mol_vae_model(encoder_rep, smi_seq_len, tgt_tokens)
...
class Molecule_VAE(nn.Module):
    def __init__(self, latent_dim, smi_dictionary, decoder=None, teacher_forcing=None, infer_method=None, **kwargs):
        ...
        self.linear_1 = nn.Linear(latent_dim, latent_dim)
        self.linear_2 = nn.Linear(latent_dim, latent_dim)
        self.linear_3 = nn.Linear(latent_dim, latent_dim)
    ...
    def decode(self, z=None, max_len=256, dec_in=None):
    ...
    def forward(self, x, max_len, dec_in=None):
        z_mean, z_logvar = self.encode(x)
        z = self.sampling(z_mean, z_logvar)
        return self.decode(z, max_len, dec_in), z_mean, z_logvar
    ...
    def inference(self, mean=None, logvar=None, condition_info=None, max_len=None, dec_in=None, eos_idx=None):
        ...
        for _ in range(max_len):
            output = self.decode(z, max_len, dec_in)
...
@register_model_architecture("vae", "vae")
...
@register_model_architecture("vae", "vae-oled")
def base_architecture(args):
    # encoder
...
@register_model_architecture("vae", "vae-opv")
def base_architecture(args):
    # encoder
    args.encoder_layers = getattr(args, "encoder_layers", 8)
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
...
@register_model_architecture("vae", "vae-laser")
def base_architecture(args):
    # encoder
    args.encoder_layers = getattr(args, "encoder_layers", 8)
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
